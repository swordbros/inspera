...
class EventSearch extends ComponentBase
{
    ...
    public function onGetEvents()
    {
        ...
        if (Input::get('queryString')) {
            $result['#autocomplete-results'] = $this->renderPartial('search/autocomplete', ['events' => $result['events']]);
        }
    ...
    {
        //TODO pass lang context with request?
        App::setLocale('en');
        ...
        return [
            ...
            'clearAllFilters' => Lang::get('swordbros.themeextender::lang.components.clear_all_filters'),
        ];
    }
    ...
    {
        $params = Input::get('params') ?? [];
        if ($qString = Input::get('queryString')) {
            $params['queryString'] = $qString;
        }
